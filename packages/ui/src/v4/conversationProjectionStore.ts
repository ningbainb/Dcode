...
const RUNTIME_RECYCLED_ERROR = "Dcode agent runtime 已被回收，重连未成功";
...
function isRuntimeRecycleError(error: unknown): boolean {
  ...
  return (
    ...
    message.includes("Dcode agent transport closed") ||
    message.includes("Dcode Protocol client disposed") ||
    message.includes("Dcode Protocol client is disposed") ||
    message.includes("Dcode Agent runtime is not running")
  );
}
