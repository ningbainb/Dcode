...
test('streamed text is visible before commit and replaced without duplication', () => {
  ...
  for (const text of ['hello ', 'world']) state = projectFrame(state, {
    type: 'assistant-stream', frame: { type: 'chunk', attemptId: 'a', chunk: { type: 'text-delta', text } },
  });
  assert.equal(state.rows[0].text, 'hello world');
  assert.equal(state.busy, true);
  state = projectFrame(state, event(0, 'assistant/message', { message: { content: [{ type: 'text', text: 'hello world' }] } }));
...
test('history reconstructs tool output and ignores duplicate durable events', () => {
  const records = [
    ...
    event(1, 'tool/result', { message: { content: [{ type: 'tool-result', toolCallId: 't', content: [{ type: 'text', text: 'exit 0' }] }] } }),
  ];
...
test('model errors settle the busy state with a readable message', () => {
  const state = projectFrame({ ...emptyProjection(), busy: true }, event(0, 'turn/end', {
...
test('reopening an active session preserves the compact streamed prefix', () => {
  const state = projectFrame(emptyProjection(), { type: 'snapshot', records: [], assistantStream: {
    activeAttempt: { attemptId: 'a', stream: [
...
for (const kind of ['plugin', 'skill-catalog']) test(`${kind} context is not presented as a human message`, () => {
  const state = projectFrame(emptyProjection(), event(0, 'user/message', {
