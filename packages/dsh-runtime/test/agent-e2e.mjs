...
backend.on('event', event => {
  events.push(event);
  if (event.type === 'approval') void backend.respondApproval(event.sessionId, event.data.requestId, true);
});
try {
  await backend.start();
  await backend.configureProvider({ provider: 'dcode-settings-fixture', api: 'openai-completions',
    baseURL: `http://127.0.0.1:${server.address().port}/v1`, model: 'dcode-fixture',
  ...
  await backend.sendMessage(session.id, 'Read input.txt, write and edit output.txt, then execute the verification command.', model);
  ...
  assert.equal(events.filter(event => event.data?.event?.type === 'tool/call').length, commands.length);
  const toolResults = events.filter(event => event.data?.event?.type === 'tool/result')
    .flatMap(event => event.data.event.data.message.content).filter(block => block.type === 'tool-result');
  assert.equal(toolResults.length, commands.length);
  assert.ok(toolResults.every(block => !block.isError), 'all coding tools must succeed');
  assert.ok(JSON.stringify(toolResults).includes('DCode input'), 'read/search must return project content');
  assert.ok(events.some(event => event.type === 'assistant-stream' && event.data.frame?.chunk?.type === 'text-delta'));
  ...
  while (!events.slice(beforeCancel).some(event => event.type === 'assistant-stream' && event.data.frame?.chunk?.type === 'text-delta')) {
  ...
  const cancelled = events.slice(beforeCancel).find(event => event.data?.event?.type === 'turn/end');
  ...
  await writeFile(join(root, 'result.json'), JSON.stringify({ passed: true, model: 'local deterministic fixture',
    runtime: 'official DSH 0.1.6-alpha.2', tools: commands.map(([name]) => name), streaming: true,
    restartHistory: true, crashRecovery: true, cancellation: true, providerSettings: true, requests: requests.length, sessionId: session.id }, null, 2));
  console.log('PASS: real DSH read/write/edit/pwsh, incremental stream, session restart/history (fixture model).');
} catch (error) {
