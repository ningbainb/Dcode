...
test('concurrent startup shares one controller and authenticates before ready', async () => {
  ...
  const server = createServer((_req, res) => {
    res.writeHead(303, { 'set-cookie': 'dcode=test; HttpOnly', location: '/' }); res.end();
  });
  await new Promise(done => server.listen(0, '127.0.0.1', done));
  let starts = 0, stops = 0;
  const controller = Object.assign(new EventEmitter(), {
    status: { state: 'stopped' },
    async start() { starts++; this.status = { state: 'ready', url: `http://127.0.0.1:${server.address().port}/launch` }; },
    async stop() { stops++; this.status = { state: 'stopped' }; },
  });
  const runtime = new DshLifecycle({ dataDir: root, controllerFactory: () => controller,
...
test('profile failure is visible and a later start can recover', async () => {
  const root = await mkdtemp(resolve(import.meta.dirname, '../../../../.dcode-lifecycle-'));
  let attempts = 0;
  const runtime = new DshLifecycle({ dataDir: root, prepareProfile: async () => {
    attempts++; throw new Error('fixture profile unavailable');
  } });
  try {
