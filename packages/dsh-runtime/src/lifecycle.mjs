...
export class DshLifecycle extends EventEmitter {
  ...
  health() {
    ...
    return {
      state: this.lastError || status?.error ? 'error'
        : status?.state === 'ready' && this.cookie ? 'ready'
        : this.starting || status?.state === 'starting' ? 'starting' : 'stopped',
      ...(this.lastError || status?.error ? { error: 'DeepSeek Harness failed to start or stopped unexpectedly. Restart Runtime or open logs.' } : {}),
  ...
  async start() {
    ...
    try { return await this.starting; }
      ...
      this.cookie = '';
      await appendFile(join(this.logsPath, 'dsh-runtime.log'), `Startup failed: ${error.message}\n`).catch(() => {});
      this.emit('status', this.health());
      throw new Error('DeepSeek Harness failed to start. Restart Runtime or open logs.', { cause: error });
    } finally { this.starting = undefined; }
  }

  async startRuntime() {
    await mkdir(this.logsPath, { recursive: true });
    const { cliPath } = await this.prepareProfile(this.dshHome);
    if (!this.controller) {
      this.controller = this.controllerFactory({
        cliPath, cwd: this.dataDir, dshHome: this.dshHome, profileName: PROFILE_NAME,
        executable: this.executable, preferredPort: 0, runtimeHost: '127.0.0.1',
        autoRestart: false,
        logStore: { append: line => appendFile(join(this.logsPath, 'dsh-runtime.log'), `${line}\n`) },
        environmentProvider: () => ({ DSH_TELEMETRY_DISABLED: '1' }),
      });
  ...
  async request(path, options = {}) {
    await this.start();
    try { return await fetch(new URL(path, this.origin), {
      ...options,
      headers: { 'content-type': 'application/json', ...options.headers, cookie: this.cookie },
      signal: options.signal ?? AbortSignal.timeout(30_000),
    }); } catch (error) {
      throw new Error('DeepSeek Harness is not responding. Restart Runtime or open logs.', { cause: error });
    }
  }
}
