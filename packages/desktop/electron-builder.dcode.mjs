...
const externalRoots = Object.entries(metadata.dependencies)
  .filter(
    ([name, version]) =>
      !version.startsWith("workspace:") && !name.startsWith("@lydell/node-pty-linux-"),
  )
  .map(([name]) => name);
const externalClosure = collectRuntimeModuleClosureEntries(externalRoots, [
  desktop,
  resolve(desktop, "../.."),
]);
...
if (missing.length)
  throw new Error(
    `Missing desktop dependencies: ${missing.map((entry) => entry.moduleName).join(", ")}`,
  );
...
export default {
  ...
  win: {
    ...base.win,
    target: ["nsis"],
    artifactName: "Dcode-Setup-${version}-${arch}.${ext}",
