...
export function verifyDcodeRuntime(runtimeRoot) {
  ...
  const result = spawnSync(executable, ['--input-type=module', '-e', `
    ...
    for (const name of ['ws', 'yaml', '@deepseek-ai/dsh/lib/bin.js', '@deepseek-ai/dsh-base/package.json', '@deepseek-ai/dsh-web-app/package.json']) {
  ...
  `, entry], { cwd: runtimeRoot, encoding: 'utf8', windowsHide: true, timeout: 60000 });
  if (result.status !== 0) throw new Error(`Packaged DSH cannot load: ${result.error?.message || result.stderr}`);
