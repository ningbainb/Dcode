...
聊天输入可将 PNG、JPEG、WebP、GIF 图片通过 DSH 原生 `session/prompt` 发送；DSH 保存附件，会话恢复时显示图片条目，点击后经同一会话的 `session/attachment` 读取。普通文件（包括文本、PDF）走独立的 DSH `fileUploads/upload` 回执，再由同一会话的 `session/prompt` 引用；当前每个文件限 10 MiB、一次最多 10 个且总计不超过 50 MiB。文件上传或发送失败保留草稿，DSH 会话恢复后显示文件条目。大文件流式上传和应用内文件预览仍待实现。
